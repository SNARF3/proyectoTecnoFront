        ...
        <v-col cols="12" md="8" lg="6">
          <v-card class="styled-card">
            ...
            <v-form v-model="formValid" ref="form">
              <v-text-field v-model="nombres" label="Nombres" :rules="[v => !!v || 'Este campo es obligatorio']" outlined
                background-color="white" class="input-field"></v-text-field>
  
              <v-text-field v-model="apellidos" label="Apellidos" :rules="[v => !!v || 'Este campo es obligatorio']"
                outlined background-color="white" class="input-field"></v-text-field>
              ...
              <v-row no-gutters align="center">
                <v-col>
                  <v-text-field v-model="contrasena" label="Contraseña" :type="showPassword ? 'text' : 'password'"
                ...
                <v-col cols="auto">
                  <v-btn icon @click="togglePasswordVisibility"
                    :aria-label="showPassword ? 'Ocultar contraseña' : 'Mostrar contraseña'" class="password-btn"
                    color="primary">
              ...
              <v-btn :disabled="!formValid" :loading="loading" color="primary" @click="registrarUsuario" block
                class="submit-btn">
  ...
  export default {
    ...
    methods: {
      ...
      async registrarUsuario() {
        this.loading = true;
        try {
          const response = await this.$http.post('http://localhost:8000/usuario/registrar', {
          ...
          if (response.data.salida) {
            const id = response.data.usuario; // Corregido: Obtener el id directamente
            this.$router.push({ path: `/interes/${id}` }); // Redirigir a Interes pasando el id
          } else {
